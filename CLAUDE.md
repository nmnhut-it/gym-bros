...
- `js/plan/` — three **PURE FUNCTIONS**, no state mutation: `generator.js` `(profile)→weekly plan`, `quick.js` `(focus, level)→single ad-hoc session`, `builder.js` `(picked exercises)→runnable day`. **`generator` + `quick` apply LEVEL_INTENSITY scaling** (beginner ×0.7, advanced ×1.3). **`builder` does NOT scale + does NOT auto-add warmup/cooldown** — when the user hand-picks an exercise (favorites, recents, browse cart), they get the exercise's natural defaults and only the bài they picked. Auto plans get scaled and bookended; ad-hoc launches stay literal.
